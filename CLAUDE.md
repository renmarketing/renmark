...
- `renmark-execute --behavior` — deterministic replay of `tests/behavioral/*.behavior.json` against recorded golden snapshots. CI-safe: no network, no token spend. Exits non-zero on any FAIL/ERROR.
- `renmark-execute --behavior --accept` — records the baseline + golden snapshots (a deliberate live step).
- `renmark-execute --behavior --judge` — opt-in LLM-as-judge escalation (~$0.15/run) that adjudicates deterministic failures. Never auto-spends: on a deterministic FAIL it prints an OFFER line only, escalating solely when `--judge` is passed.
- Behavioral tests prove a skill CHANGES agent behavior (baseline-without-skill must differ from golden-with-skill), complementing the structure audit, which only lints.
