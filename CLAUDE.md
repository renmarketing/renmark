...
- `renmark-execute --behavior --accept` records the **eval tier** golden transcripts (a deliberate live step).
- `renmark-execute --behavior --judge` runs the **eval tier** — the load-bearing behavioral proof: an LLM-as-judge over a real model trajectory (~$0.15, opt-in only, never auto-spends, out of CI). On a deterministic FAIL the CLI prints an OFFER line and escalates solely when `--judge` is passed.
