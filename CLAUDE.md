...
Commit as soon as a logical chunk passes its check. One commit per logical fix/feature; commit before the next agent dispatch; each commit must pass lint; messages name the change, not the session ("fix(auth): handle 401").
...
(4) change, re-run, compare pass counts. If tests regress, revert targeted files.
...
(`features.md`, `bugs.md`, `decisions.md`, `learnings.md`), and artifact metadata.
...
`artifact_type`, `schema_version`, `created_at`, `source_sha`, `related_plan`,
`generator`, `stale_after` (optional), `dependency_refs`. Artifacts without
freshness/provenance metadata are untrusted as upstream context. Prefer invalidation over silent drift; don't solve context rot with artifact rot.
...
All executor outputs MUST expose: `completion_state`, `confidence`,
`validation_status`, `retry_count`, `parser_success`, `schema_compliance`.
