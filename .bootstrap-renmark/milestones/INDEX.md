...
## Phase 3 (partial — minimal canonical ledger)

| Release | Title | Type | Built from | Lifecycle status |
|---|---|---|---|---|
| **R-0.3** | Minimal Canonical Ledger | feature | narrowed subset of Phase 3 (Work Order, Work Result, Inspection Report, Escalation only — blueprint/milestone-contract/closeout schemas, hashing, snapshot/restore, migration adapters deferred) | **RELEASED** (2026-08-01, v0.39.6). Contract at `.renmark/plans/2026-08-01-r-0.3-minimal-canonical-ledger-contract.md`. WP-1–WP-3 (`renmark/ledger.py` — schemas, append-only writer, reader, commit `98aaf1a`), WP-4 (one real emission call site per kind in `_engine.py`/`scan.py`/`state/pause.py`, commit `c82409d`), WP-5 (21 tests total, folded into WP-1–4), WP-6 (independent review, commit `65c0139`). Acceptance: **ACCEPTED, no follow-up** — R-0.3's narrow scope closed clean on the first pass; R-0.2's F1/F2/F4/F5 stayed tracked-only and none blocked the ledger, per Owner's scope discipline at kickoff. See `.renmark/reviews/2026-08-01-r-0.3-closeout.md`. Remainder of Phase 3 (blueprint/milestone-contract/closeout schemas, hashing, snapshot/restore, migration adapters, universal dispatch-path rewiring) stays a working title for a future release. |

Phases 3 (remainder)–8 (full ledger/hashing/snapshot, minimal Inspector, Planner altitude split, deterministic integration, WritingMate, advanced optimizations): no releases drafted yet. Per addendum-01 §9 / addendum-03: only one blocking release is actively worked at a time, and no future release is broken down until the preceding blocking release closes (reaches CLOSED on the 10-state lifecycle, not merely ENGINEERING VERIFIED).
