...
**Files changed:**
- `tests/behavioral/*.behavior.json` — 9 new.
- `renmark/behavior.py` — 9 new adapters + `_DISPATCH` entries (3 waves).
- `.github/workflows/test.yml` — CI step added.
- `tests/test_behavior_ci_gating.py` — new.
**Do not change:**
- Every adapter must call real production code — never hand-copy or approximate the logic it's proving.
- The deterministic tier must stay zero-token-spend, no-network by default; CI-gating enforces the existing contract more strictly, never relaxes it.
**Release 15 complete.** Also mid-flight: hit and resumed past a session usage-limit pause after Wave 1's first 6 of 9 fixture tasks (recorded via `usage.classify_usage_pause`, resumed cleanly with `/renmark:orchestrate --resume`). AC-8 (Req 8) closes.
...
**Files changed:**
- `tests/test_release11_dispatch_scheduling.py` — new.
- `.renmark/rethink/governed-orchestration-assurance/release-11-overhead-measurement.md` — new.
**Do not change:**
- `group_tasks_by_wave(tasks)` must remain byte-identical when no new scheduling signals are supplied — the regression test enforces this.
**Release 11 complete, with one real open item.** AC-9 (Req 9) partially closed: scheduling capability shipped, tested, and measured under budget — but not yet wired into any live caller (`renmark/cli/_wave_loop.py`, `renmark/cli/_engine.py` still call the old no-arg form). A follow-up wiring task is needed before the CLI/codex loop actually benefits from `max_parallelism`/`quota_view`/`rework_lookup`/`risk_resolver` — logged here, not silently claimed as fully wired.

## [2026-08-05] — feat: rethink Release 11 tasks 1+2 (governed-orchestration-assurance) — routing-overlap spike + policy-aware scheduling
**Request:** Tasks 1 and 2 of 4, Release 11 — confirm no codex_routing.py/global_routing.py overlap, and extend `group_tasks_by_wave` (the shared, universally-live scheduling entry point) with max-parallelism, quota, rework, and risk signals.
**Built:** Spike confirmed no overlap — `codex_routing.py` is per-task model routing, `global_routing.py` is an unrelated CLAUDE.md/AGENTS.md installer; name collision only. `group_tasks_by_wave` gains 4 keyword-only, default-`None` params (`max_parallelism`, `quota_view`, `rework_lookup`, `risk_resolver`) — a no-arg call is byte-identical to the legacy path (early-return guard). Policy order: rework annotate-only (log-only, never blocks — `recurrence.pre_attempt` stays the sole blocking authority) → stable highest-risk-first intra-wave ordering → consecutive sub-wave splitting for max-parallelism/quota. Tasks are only ever serialized, never dropped or moved across waves. `dispatch_wave` gains `max_parallelism` as an alias onto the existing `max_workers` path. Full suite: 2051 passed (compat + scope-enforcement sets green), no regressions.
**Files changed:**
- `.renmark/rethink/governed-orchestration-assurance/release-11-routing-overlap-spike.md` — new.
- `renmark/dispatch.py` — policy-aware scheduling params.
**Do not change:**
- `recurrence.pre_attempt` stays the sole retry-blocking authority — `rework_lookup` is scheduling awareness only, never a second gate.
- A no-arg `group_tasks_by_wave(tasks)` call must remain byte-identical to pre-Release-11 behavior.
**Deferred findings (real, not blocking, flagged by the implementer):** neither `renmark/cli/_wave_loop.py` nor `renmark/cli/_engine.py` actually calls `group_tasks_by_wave`/`dispatch_wave` with the new params yet — the capability exists but no live caller opts in (same shape as the pre-Release-6 F1 gap; a follow-up wiring task is needed for the CLI/codex loop to actually benefit). `usage.build_usage_view`'s `limit_exceeded` is a single global bool, not per-provider — the implementer derives per-provider throttling from `percent[provider][...]` and falls back to throttling both providers conservatively when that can't be parsed. `risk_resolver` has near-zero effect on the CLI path today since `dispatch_wave` re-sorts results by task index after concurrent execution — do not advertise it as real prioritization until a consumer honors wave order.
...
**Files changed:**
- `renmark/dispatch.py` — new functions + additive field.
**Do not change:**
- This is the concrete F1 fix but the new API is STILL UNREACHABLE from production until task 9 wires it into `orchestrate/SKILL.md` — do not report F1 closed until task 9 lands.
**Implementer's notes carried forward (non-blocking):** `_maybe_scoped_claude_dispatch` requires a `repo` param that its callee never actually reads (verified) — `Path('.')` is passed with a comment; flagged as latent fragility if that callee ever gains real repo I/O. `HostDispatchPlan` is `frozen=True` but the new dict field makes immutability shallow (mutated in place) — no new hash regression since `task_packets` was already unhashable. The wrapper's `scoped_dispatches` stores `WorkerScope` (matching the task-6 test contract), while `WaveResult`'s field stores `AgentDispatch` — deliberately different types; a future unification must not assume they match.

## [2026-08-05] — test: rethink Release 6 tasks 5-7 (governed-orchestration-assurance) — envelope tests + baseline enforcement tests (retry) + hook Bash tests
**Request:** Tasks 5, 6, 7 of 9, Release 6 — test coverage for `check_capability_envelope`, the required test-before-flip-switch baseline for Task 8's dispatch-path wiring, and hook-hardening coverage.
**Built:** `tests/test_subagent_gate_capability_envelope.py` — 17 tests covering all 6 control dimensions, the status-table lock, and codex's post-action-only path phrasing. `tests/test_r6_host_agent_scope_enforcement.py` — 3 tests against the API Task 8 will add (`build_host_dispatch_plan_with_scope`, `enforce_host_agent_dispatch_scope`); currently fails to collect on the not-yet-existing import, the CORRECT pre-Task-8 state. `tests/test_capability_envelope_prototype.py` — 4 new Bash-hardening tests, plus a fix to `test_non_write_tool_defers` (its `ls`/Bash example predated Bash-command judging; switched to `Grep`, which genuinely still defers, preserving the test's original intent).
**Files changed:**
- `tests/test_subagent_gate_capability_envelope.py` — new.
- `tests/test_r6_host_agent_scope_enforcement.py` — new (task 6 retry — see process note).
- `tests/test_capability_envelope_prototype.py` — 4 new tests + 1 fixed test.
**Do not change:**
- Do not stub `build_host_dispatch_plan_with_scope`/`enforce_host_agent_dispatch_scope` to make task 6's file collect early — Task 8 implements them for real; the current collection failure is intentional TDD state.
**Process note:** task 6's first attempt (codex ad-hoc `--task/--output` path) again produced a bogus artifact-report document instead of real code — the 3rd occurrence of this exact failure mode this session (see `.renmark/memory/learnings.md`), and this time on a NEW file, not an extend — revising the earlier "extend-only" theory. Restored and redispatched via Sonnet. Task 5, dispatched via the same codex path in the same batch, was genuinely fine (17 real, correct tests) — the failure is intermittent (~50% observed this session), not deterministic per task shape. Recommendation logged: avoid the ad-hoc codex `--task` path for this program's remaining releases; always independently re-verify before trusting a self-reported PASS.
...
**Files changed:**
- `tests/test_task_tracking_engine_wiring.py` — new end-to-end test.
**Do not change:**
- **Process finding, logged to `.renmark/memory/learnings.md`:** the FIRST attempt at this task, dispatched via `renmark-execute --task <brief> --output <file>` (the codex ad-hoc single-task path), overwrote the entire existing test file with a bogus YAML-frontmatter "artifact report" document instead of extending it with real Python — a `SyntaxError` on the very first line, while self-reporting `status: "PASS"` in its own output despite its own report text admitting "This pass did not execute the verifier." Caught only because the orchestrator independently re-ran the verifier rather than trusting the self-reported status. File was restored via `git checkout <last-good-sha> -- <file>` and the task was redispatched via a Sonnet Agent-tool call with an explicit "this is a real Python test file, do not write frontmatter/prose" instruction, which succeeded cleanly. A milder version of the same failure shape occurred on Release 3 task 5 (codex wrapped the same bogus frontmatter in a docstring, which happened to keep the file syntactically valid — masking the same underlying unreliability). **Recommendation for future releases in this program:** prefer Sonnet over the `renmark-execute --task` codex ad-hoc path for "extend an existing test file" tasks; codex's whole-plan wave path (used for tasks 1-4 of Release 1, and Release 4 task 4/6) has not shown this failure mode — only the single-task ad-hoc `--task`/`--output` invocation has, twice.
...
**Files changed:**
- `.renmark/memory/orchestration-baseline.md` — new Scenario capture section.
**Do not change:**
- Do not fabricate numbers for the still-`unknown` Start/Rethink-final rows — they require an explicit, separately-approved fresh pipeline run.
...
**Built:** `release.compact_snapshot_dir()` (retires one snapshot dir to `<name>.meta/`, keeping only `manifest.json`/`release.md`/`verification.md`/`files-changed.txt`) and `release.compact_previous_snapshots()` (finds every OTHER full snapshot dir under the version dir and compacts it). `build_version_snapshot()` now calls `compact_previous_snapshots(keep=<current snap name>)` at the end of every build, so this runs automatically on every future release — no separate wiring into `finish_lanes.py` needed since every release path already funnels through `build_version_snapshot`. Retroactively compacted the live `v0.41.0/` → `v0.41.0.meta/`. 4 new regression tests (compacts prior version on bump, leaves current/already-`.meta` dirs untouched, missing-meta-file no-raise). Full suite green (1962 passed, 31 skipped), ruff + mypy clean.
**Files changed:**
- `renmark/release.py` — `SNAPSHOT_META_FILES`, `compact_snapshot_dir`, `compact_previous_snapshots`; `build_version_snapshot` calls the latter and returns a new `compacted` key.
- `tests/test_release_snapshot.py` — 4 new tests.
- `.renmark/version/v0.41.0/` → `.renmark/version/v0.41.0.meta/` (retroactive one-time compaction).
**Do not change:** compaction identifies snapshot dirs by the presence of `manifest.json` (not by `v<ver>` naming) so it stays safe for the `archive_stem` maintainer override — don't narrow that check to a strict `v<ver>` regex, it would stop compacting custom-named snapshot dirs.
...
**Files changed:** `renmark/plan_lint.py` (+33 lines, Check 13 + wiring), `tests/test_plan_lint.py` (+45 lines, 3 new tests).
**Do not change:** `domain_of()`'s "never raises" contract in `lifecycle/stage.py` is unchanged — this is a lint-time check only, zero runtime behavior change. Full `pytest -q`: 1939 passed/31 skipped (+3 from the new tests). None of the three protected areas touched.

**Rethink roadmap status: all 7 releases addressed.** Release 1 partial (compat tests done; live numeric baseline measurement deliberately deferred — needs a dedicated, materially larger session). Releases 2-7 complete, with two scope revisions made explicit and Owner-approved along the way rather than silently absorbed: Release 3's `delivery_state.py` stays permanently stdlib-only, and Release 6 was scoped down from a full 3-way routing merge to the genuinely duplicated pricing table (which turned out to have already drifted into a live bug, now fixed).

## [2026-08-04] — fix: rethink Release 6 (scoped) — centralize executor pricing table on renmark.cost.PRICE_PER_KTOK (REQ-2/REQ-30 exception)
**Request:** After the PRD gate approval (previous entry), inspected the actual code behind Release 6's "centralize routing" goal and found the blueprint's 3-way merge (model tier / subagent role / Codex dispatch shape into one `resolve_executor()`) conflates distinct decisions, not genuine duplication — scoped down per Owner decision to only what's actually duplicated: the executor cost-per-1k-tokens pricing table, which existed as **3 independent copies** (`renmark/cost.py`'s `PRICE_PER_KTOK`, a local dict in `renmark/cli/_engine.py`, and `renmark/roadmap.py`'s `COST_PER_KT`) that had **already drifted** — `codex` priced at `$0.03` in `cost.py` but `$0.05` in the other two. Confirmed `$0.03` is correct with the Owner before touching anything (not guessed).
**Built:** `renmark/cli/_engine.py`'s dry-run cost-preview function now imports `renmark.cost.PRICE_PER_KTOK` instead of hand-rolling a local copy. `renmark/roadmap.py`'s `COST_PER_KT` now derives from the same canonical table (`{**PRICE_PER_KTOK, "nim": 0.0}`, keeping the legacy `nim` key that `cost.py` doesn't carry so old `[nim]`-tagged commits still resolve to a defined rate). `cost.py` remains a leaf (zero `renmark.*` imports) — both callers depend on it, not the reverse, no new import cycle. `tests/test_roadmap.py::test_write_roadmap_md` was asserting the old, incorrect `$0.075` (1500 tokens × the stale $0.05/kT) — corrected to `$0.045` (the canonical $0.03/kT), a necessary consequence of fixing the drift, not a workaround. Full `pytest -q`: 1936 passed/31 skipped (unchanged pass count; one assertion value corrected). `ruff check` clean.
**Files changed:** `renmark/cli/_engine.py`, `renmark/roadmap.py`, `tests/test_roadmap.py`.
**Do not change:** `renmark.cost.PRICE_PER_KTOK` is now the sole source of truth for executor pricing — do not reintroduce a local copy in any caller. The full 3-way `resolve_executor()` merge (model tier + subagent role + Codex dispatch shape) described in target-blueprint.md §1.4 was deliberately NOT done — Owner decision was to keep those as separate, distinct-purpose functions rather than risk conflating them; this scoped fix satisfies the PRD-note's documented benefit (drift elimination) without the higher-risk full merge. None of the three role-model/selector/task-tracking protected areas were touched.

## [2026-08-04] — PRD updated: pre-authorize rethink Release 6 as a REQ-30 routing-change exception
**Request:** Release 6 of the rethink roadmap (centralize routing behind `cost.resolve_executor`) touches orchestration routing across ~12 files. REQ-30(i) requires an explicit PRD change and Owner approval through this UPDATE gate for any such change, with no carve-out for outcome-neutral consolidation — flagged before writing any Release 6 code rather than treated as pre-approved by the rethink roadmap's own Execution Gate (a different, non-substitutable gate).
**Built:** Added a Revision note to `PRD.md` (dated 2026-08-04) invoking REQ-30's existing regression-protection exception path (quantified evidence + Owner approval + documented benefit + rollback path) specifically for Release 6. Documents the benefit (4+ touch points → 1 seam), the proof obligation (a golden before/after routing-decision test showing byte-identical tier selection, required before any call site is migrated), the no-new-cycle constraint (`cost.py` stays a leaf), and the rollback path. Bumped `last_reviewed` to 2026-08-04.
**Files changed:** `PRD.md` — new Revision note authorizing Release 6, `last_reviewed` bumped.
**Do not change:** Release 6 implementation must still produce the golden before/after routing-decision test as its own proof — this PRD note authorizes the *approach*, it does not substitute for that evidence. REQ-2's routed executor set and REQ-30's other guarantees (Owner-gate frequency, dispatch count, context limits) are explicitly unaffected and must stay that way.

## [2026-08-04] — refactor: rethink Release 5 complete (5/5) — final verification pass
**Request:** Finishing Release 5 — the consolidated verification pass across the whole `lifecycle/` package (done directly, no subagent needed — this step is verification/wrap-up, not code movement).
**Built:** Ran the full acceptance criteria from `.renmark/state/program.json`'s `r5-split-lifecycle` task: full `pytest -q` — 1936 passed/31 skipped (unchanged from the pre-Release-5 baseline across all 4 steps); `mypy renmark/lifecycle/` — 0 new errors (the 1 reported error is `renmark/ledger.py`, pre-existing and untouched); `ruff check renmark/lifecycle/` — clean; `STAGES` and `LIFECYCLE_JSON_BYTE_BUDGET` (1024 bytes = 1KB) confirmed byte-for-byte identical to pre-split; ran `tests/test_cross_host_dispatch_e2e.py` + `tests/test_hosts.py` + `tests/test_lifecycle.py` + `tests/test_next_steps.py` directly (124 passed) as the host-parity check; and directly called `lifecycle.skill_preamble('.', 'orchestrate', host='claude')` vs. `host='codex'` — outputs byte-identical, satisfying the roadmap's own Owner acceptance scenario ("skill_preamble/resume behavior identical across both hosts post-split").
**Result:** `renmark/lifecycle.py` (1747 lines, 1 file, the project's highest bug-history module) is now `renmark/lifecycle/` (5 files, 1982 lines total): `stage.py` (1057L), `next_steps.py` (175L), `preamble.py` (324L), `reconciliation.py` (297L, the isolated staleness hotspot), `__init__.py` (129L, backward-compat re-export shim with monkeypatch-forwarding). Every `from renmark.lifecycle import X` call site across the repo kept working unchanged throughout all 5 steps.
**Files changed:** none this step (verification only).
**Do not change:** the `_LifecyclePackage` `ModuleType` subclass in `__init__.py` is load-bearing — it's what lets `monkeypatch.setattr(lifecycle, ...)` keep reaching the right implementation module across 4 separate files instead of 1. Any future split of `stage.py`/`next_steps.py`/`preamble.py`/`reconciliation.py` further must extend `_IMPL_MODULES` the same way. None of the three Owner-protected areas were touched across any of Release 5's 5 steps.

## [2026-08-04] — refactor: rethink Release 5 (step 4/5) — extract lifecycle/reconciliation.py (the bug-history hotspot)
**Request:** Continuing Release 5's incremental lifecycle-package split, step 4 — the actual point of this release: isolate the cross-store reconciliation logic that produced 3+ documented staleness bugs across R-0.1/R-0.2/R-0.3 into its own focused file, per target-blueprint.md §1.2 (Stage 5 §4 finding).
**Built:** Moved `milestone_signoff_readiness`, `read_legacy_delivery_summary`, `_project_workflow_delivery`, `_workflow_drift_notes` verbatim out of `stage.py` into `lifecycle/reconciliation.py` — logic relocated, not rewritten (explicitly instructed not to "fix" anything that looked off during the move, only flag it; nothing was flagged). No circular back-reference needed. `_IMPL_MODULES` in `lifecycle/__init__.py` updated to `(_stage, _next_steps_mod, _preamble_mod, _reconciliation_mod)`. The subagent's own final report was truncated mid-task again (third time in this series) — did not trust it, independently re-verified everything from scratch: full `pytest -q` 1936 passed/31 skipped (unchanged), `milestone_signoff_readiness`/`read_legacy_delivery_summary` resolve correctly, `ruff check renmark/lifecycle/` clean, `_IMPL_MODULES` confirmed with all 4 modules, live `renmark-execute --dry-run` smoke test passed.
**Files changed:** `renmark/lifecycle/reconciliation.py` (new, 297 lines), `renmark/lifecycle/stage.py` (1299 → 1057 lines), `renmark/lifecycle/__init__.py` (116 → 129 lines).
**Do not change:** the reconciliation logic itself was moved, not audited or fixed — its known bug history is now isolated to one 297-line file (down from a 1700+-line module), which is the structural risk-reduction outcome this release targeted, but the logic's correctness is unchanged from before this release. A future staleness bug fix should target `reconciliation.py` directly. None of the three Owner-protected areas were touched. 1 of 5 lifecycle-split steps remains: a final consolidated verification pass across the whole package.

## [2026-08-04] — refactor: rethink Release 5 (step 3/5) — extract lifecycle/preamble.py
**Request:** Continuing Release 5's incremental lifecycle-package split, step 3: extract the preamble/checkpoint cluster (`skill_preamble` — likely the single most externally-referenced function in the whole module — `preamble_tier`, `persist_compact_checkpoint`, `milestone_context_checkpoint`, `_with_mode_note`, `_with_agency_note`, `_with_headless_note`) out of `stage.py`, per target-blueprint.md §1.2.
**Built:** Moved all 7 names verbatim; no circular back-reference needed (nothing left in `stage.py` calls into this cluster, confirmed by grep). `_IMPL_MODULES` in `lifecycle/__init__.py` updated to `(_stage, _next_steps_mod, _preamble_mod)`. The subagent's own final report was truncated mid-sentence (the same odd behavior seen once before, in Release 4 part 1) — did not trust it, independently re-verified everything from scratch: full `pytest -q` 1936 passed/31 skipped (unchanged), all 4 key functions resolve correctly via `from renmark import lifecycle`, `ruff check renmark/lifecycle/` clean, `_IMPL_MODULES` correctly extended, and a live `renmark-execute --dry-run` smoke test passed end-to-end.
**Files changed:** `renmark/lifecycle/preamble.py` (new, 324 lines), `renmark/lifecycle/stage.py` (1595 → 1299 lines), `renmark/lifecycle/__init__.py` (107 → 116 lines).
**Do not change:** None of the three Owner-protected areas were touched. 2 of 5 lifecycle-split steps remain: `reconciliation.py` (the actual bug-history hotspot this whole release exists to isolate) and a final verification pass.

## [2026-08-04] — refactor: rethink Release 5 (step 2/5) — extract lifecycle/next_steps.py
**Request:** Continuing Release 5's incremental lifecycle-package split. Step 2 extracts the "next steps" cluster (`NextSteps`, `next_steps`, `next_recommended`, `_resolve_next`, `_gates_not_run`) out of `stage.py` into `lifecycle/next_steps.py`, per target-blueprint.md §1.2.
**Built:** Moved the 5 names verbatim (151 lines out of `stage.py`); call-graph survey confirmed no circular back-reference needed (nothing left in `stage.py` calls back into this cluster). Updated `lifecycle/__init__.py`'s `_IMPL_MODULES` tuple to include the new module (required per step 1's own warning — every extraction must self-register or the monkeypatch-forwarding shim silently stops covering the moved names). One real trap caught and handled: the submodule is named `next_steps.py` and also exports a function called `next_steps` — import ordering in `__init__.py` matters so `lifecycle.next_steps` resolves to the function (which `renmark/behavior.py:414` calls directly), not the submodule. Independently re-verified before commit: full `pytest -q` 1936 passed/31 skipped (unchanged), `ruff check renmark/lifecycle/` clean, `lifecycle.next_steps`/`next_recommended`/`NextSteps` all resolve correctly, `_IMPL_MODULES` attribute-forwarding confirmed working for names in both modules.
**Files changed:** `renmark/lifecycle/next_steps.py` (new, 175 lines), `renmark/lifecycle/stage.py` (1747 → 1595 lines), `renmark/lifecycle/__init__.py` (100 → 107 lines).
**Do not change:** the `_next_steps_mod` import must stay before the `next_steps` function import in `__init__.py` — reversing the order would make `lifecycle.next_steps` resolve to the submodule instead of the function, breaking `renmark/behavior.py`'s call site. None of the three Owner-protected areas were touched. 3 of 5 lifecycle-split steps remain: `preamble.py`, `reconciliation.py`, and a final verification pass.

## [2026-08-04] — refactor: rethink Release 5 (step 1/5) — convert renmark/lifecycle.py into a package (mechanical move, no logic split)
**Request:** Starting Release 5 (`.renmark/rethink/renmark-architecture/roadmap.md`, target-blueprint.md §1.2): split the 1747-line `renmark/lifecycle.py` — the project's highest bug-history file (3+ documented reconciliation staleness bugs across R-0.1/R-0.2/R-0.3) — into a `lifecycle/` package (`stage.py`, `next_steps.py`, `preamble.py`, `reconciliation.py`). Given the size and history, split into 5 smaller dispatched steps instead of Release 4's 3, starting with a pure mechanical file→package move (zero logic change) to de-risk before the 4-way logical split.
**Built:** `git mv renmark/lifecycle.py renmark/lifecycle/stage.py` (verbatim, history-preserving) + new `renmark/lifecycle/__init__.py` re-export shim. 23 relative imports inside the moved file (`from .x` → `from ..x`) needed depth adjustment since `.` now resolves to `renmark.lifecycle` instead of `renmark` — the only non-verbatim edit, purely mechanical. Discovered and solved a real correctness trap: ~25 tests do `monkeypatch.setattr(lifecycle, "_some_internal_name", ...)` then call a `lifecycle.py` function that reads that name from its *own* module globals — a plain `from .stage import *` re-export would make the package and the impl module separate namespaces, silently breaking every such monkeypatch (tests would still pass against unpatched real behavior, a silent correctness regression, not a loud failure). Fixed with a `ModuleType` subclass (`_LifecyclePackage`) whose `__setattr__`/`__getattr__`/`__delattr__` forward to the implementation module(s) listed in `_IMPL_MODULES`, restoring pre-split single-module attribute semantics exactly. Independently re-verified before commit: full `pytest -q` 1936 passed/31 skipped (unchanged, `tests/test_lifecycle.py` included), `ruff check renmark/lifecycle/` clean, `python3 -c "from renmark import lifecycle"` resolves `STAGES`/`read_lifecycle`/`write_lifecycle`/`skill_preamble`/`next_steps` correctly.
**Files changed:** `renmark/lifecycle.py` → `renmark/lifecycle/stage.py` (renamed, 1747 lines, git-tracked as a move), `renmark/lifecycle/__init__.py` (new, 101 lines).
**Do not change:** `_IMPL_MODULES` in `lifecycle/__init__.py` currently lists only `stage.py` — **every subsequent extraction (next_steps.py, preamble.py, reconciliation.py) MUST append itself to `_IMPL_MODULES`**, or the monkeypatch-forwarding trick silently stops covering names that move to the new file and the ~25 dependent tests regress without an obvious error pointing at the cause. None of the three Owner-protected areas were touched.

## [2026-08-04] — refactor: rethink Release 4 complete (3/3) — extract renmark/cli/_wave_loop.py (REQ-30, exempt)
**Request:** Finishing Release 4 — the last and biggest of the three `_engine.py` extractions: `execute_plan()`'s ~394-line dispatch-loop body, per target-blueprint.md §1.1's explicit two-step instruction (first decompose into named helpers in place, then relocate).
**Built:** Dispatched to a third opus subagent with the same checkpoint discipline as part 2, plus extra emphasis given `execute_plan` is renmark's own live dispatch path (this session's own `renmark-execute` calls run through it). It decomposed the body into `_prepare_waves`, `_create_parent_task`, `_emit_work_order`, `_track_worker_dispatch`, `_emit_work_result`, `_inspect_and_track`, `_run_waves` (+ `_WaveRunOutcome`), verified a clean checkpoint, then relocated those plus `_process_wave_results` into the new `renmark/cli/_wave_loop.py`. `execute_plan`/`main` stay in `_engine.py` as thin shells. Key correctness catch from its pre-move grep: several tests monkeypatch `_execute_task`/`_r008_precheck`/`parse_plan`/`codex_available`/`_execute_task_codex` on the `_engine` module namespace — the relocated code reaches those through the established `_get_engine()` lazy accessor so existing monkeypatches still take effect. Independently re-verified myself before committing: `py_compile` clean, `from renmark.cli import _engine` + `execute_plan` callable, full `pytest -q` 1936 passed/31 skipped (unchanged), `ruff check` same 4 pre-existing issues only, and a live `renmark-execute --dry-run` smoke test against a real plan file resolved end-to-end correctly through the new module split.
**Files changed:** `renmark/cli/_engine.py` (1111 → 788 lines; started at 1698), `renmark/cli/_wave_loop.py` (new, 569 lines).
**Release 4 is now DONE (3/3).** Total: `_engine.py` split from 1698 lines into a 788-line shell + `_dispatch_flags.py` (437) + `_run_lifecycle.py` (271) + `_wave_loop.py` (569). None of the three Owner-protected areas were touched across any of the three extractions; no new architectural conflict surfaced.
**Do not change:** the `_get_engine()` lazy-accessor pattern in `_dispatch_flags.py`/`_run_lifecycle.py`/`_wave_loop.py` exists specifically so test monkeypatches on `renmark.cli._engine`'s module namespace keep working — do not "clean this up" into direct imports without re-checking every test that patches `_engine.<name>` first.

## [2026-08-04] — refactor: rethink Release 4 (partial 2/3) — extract renmark/cli/_run_lifecycle.py (REQ-30, exempt)
**Request:** Continuing Release 4 (`.renmark/rethink/renmark-architecture/roadmap.md`), part 2 of the 3-way `_engine.py` split. Extracted the "run-bookkeeping cluster" per target-blueprint.md §1.1: `_setup_resume_state`, `_begin_run_state`, `_complete_clean_run`, `_handle_run_exit`, `_print_run_summary`.
**Built:** Dispatched to a second opus subagent, this time with an explicit checkpoint discipline (stop clean or finish clean, never half-moved) after part 1's dispatch left a broken intermediate state. This dispatch completed cleanly and self-verified (baseline-vs-final pytest and ruff counts). Independently re-verified myself before committing (did not just trust the report): `python3 -m py_compile` + `from renmark.cli import _engine` both clean, full `pytest -q` 1936 passed/31 skipped (unchanged), `ruff check` shows the same 4 pre-existing baseline issues and zero new ones. Two of the moved functions' internal dependencies on `_engine.py`-local helpers (`_print`, `_git_tag`, `_cross_check_skip_list`) stayed reached via the existing `_get_engine()` lazy-accessor pattern (same template `_dispatch_flags.py` already used) rather than duplicated or hard-imported, preserving test-monkeypatch targets on `_engine` unchanged.
**Files changed:** `renmark/cli/_engine.py` (1316 → 1111 lines), `renmark/cli/_run_lifecycle.py` (new, 271 lines).
**Do not change:** This is 2 of 3 planned extractions — `_wave_loop.py` (execute_plan()'s ~394-line body) is not done; `_engine.py` is still 1111 lines. `.renmark/state/program.json`'s `r4-split-cli-engine` task stays `partial`. None of the three Owner-protected areas were touched; no new architectural-invariant conflict surfaced (the delivery_state/lifecycle imports these functions need are reached exactly the way they always were).

## [2026-08-04] — refactor: rethink Release 4 (partial 1/3) — extract renmark/cli/_dispatch_flags.py (REQ-30, exempt)
**Request:** Continuing the Owner-approved rethink roadmap. Release 4 splits the 1698-line `renmark/cli/_engine.py` into a thin shell + 3 cohesive modules (`_dispatch_flags.py`, `_run_lifecycle.py`, `_wave_loop.py` per target-blueprint.md §1.1/§3.6).
**Built:** Dispatched to an opus subagent (code-implementer role) with strict behavior-preservation instructions. The agent extracted the first module — `renmark/cli/_dispatch_flags.py` (437 lines: `_cmd_behavior`, `_behavior_accept`, `_judge_est_cost`, `_bounded_delivery_state_display`, `_delivery_state_line`, and the six `_dispatch_*_flags` functions) — but was interrupted mid-task (240s/27 tool calls) before wiring the re-export back into `_engine.py`, leaving the working tree in a genuinely broken intermediate state (functions removed from `_engine.py` with no import replacing them; the main dispatch loop's lambda list referenced now-undefined names).
**Fixed directly (not re-dispatched):** Built a coverage inventory via grep (which of the 11 extracted names are actually referenced from `_engine.py`'s own body vs. only externally via tests) and added the correct re-export import block, `as`-aliasing the 5 names with zero internal callers (pure backward-compat re-exports, e.g. `tests/test_cli_delivery_state.py`'s direct `from renmark.cli._engine import _delivery_state_line`) so ruff doesn't flag them unused. Ran `ruff check --fix` (11 of 15 flagged issues were import-sort noise from the new block; the remaining 4 are pre-existing, confirmed identical via `git stash` against the pre-Release-4 baseline). Full `pytest -q`: 1936 passed, 31 skipped — unchanged from before this extraction.
**Files changed:** `renmark/cli/_engine.py` (1698 → 1316 lines), `renmark/cli/_dispatch_flags.py` (new, 437 lines).
**Do not change:** This is 1 of 3 planned extractions — `_run_lifecycle.py` and `_wave_loop.py` are NOT done; `_engine.py` is still 1316 lines, well above the "thin shell" target. `.renmark/state/program.json`'s `r4-split-cli-engine` task is marked `partial`, not `done`. None of the three Owner-protected areas were touched. **Process note for future sessions:** a single large subagent dispatch for this kind of file-split is fragile — verify the actual diff and re-run the full suite yourself before trusting an agent's self-reported completion; do not chain a second big extraction onto an unverified first one.

## [2026-08-04] — decision: rethink Release 3 scope revised — delivery_state.py stays permanently stdlib-only, Release 3 complete
**Request:** Owner decision on the conflict flagged in the prior entry: keep `delivery_state.py`'s documented "stdlib-only and intentionally self-contained" invariant, revise the rethink roadmap to record that boundary as a permanent architectural constraint rather than deferred work, and mark Release 3 complete (not partial).
**Built:** Updated `.renmark/rethink/renmark-architecture/roadmap.md`'s Release 3 section with a "Scope revision (Owner decision, 2026-08-04)" note recording the decision and the revised acceptance criterion (schemas.py has zero `renmark.*` imports except the one documented, permanent exception for `delivery_state.py`'s constants). `.renmark/state/program.json`'s `r3-invert-schemas-imports` task and `release-3-schemas-dependency-reversal` stage flipped from `partial` to `done`.
**Files changed:** `.renmark/rethink/renmark-architecture/roadmap.md`, `.renmark/state/program.json` (runtime state, not committed to git).
**Do not change:** `delivery_state.py`'s stdlib-only guarantee is now a permanent design constraint, not a TODO — a future session must not "finish" the constant move without a fresh, explicit Owner decision reopening this one.

## [2026-08-04] — refactor: rethink Release 3 (partial) — reverse schemas.py's inverted dependency for STAGES/SUBAGENT_OUTPUT_* (new)
**Request:** Owner said "continue to the next" then "go ahead and implement all the ones you had plan" — proceeding through the rethink roadmap's Release 3 (`.renmark/rethink/renmark-architecture/roadmap.md`): make `renmark/schemas.py` the sole owner of shared contract constants instead of importing them from the modules that need its validators (an inversion that had already forced function-local workaround imports in `lifecycle.py`/`dispatch.py` to dodge circular imports — visible in their own code comments before this change).
**Built:** Moved `STAGES` (from `renmark/lifecycle.py`) and the four `SUBAGENT_OUTPUT_*` constants (from `renmark/dispatch.py`) into `renmark/schemas.py`; both modules now import them back from `schemas.py` at module level, and their prior function-local `schemas` workaround-import comments were corrected to describe the real remaining reason (schemas needs the fully-loaded caller module for validation calls, not a constants cycle). Constant **values** are unchanged — this is purely an import-direction fix.
**Discovered conflict — NOT implemented, flagged instead of silently overridden:** the roadmap also called for moving `CONTRACT_VERSION`/`SCHEMA_VERSION`/`PROVENANCE_EVENT_CAP`/`WORK_PACKAGE_CAP`/`LEGACY_REF_CAP`/`SUMMARY_TEXT_LIMIT`/`stable_milestone_id`/`stable_work_package_id` out of `renmark/delivery_state.py` the same way. `delivery_state.py`'s own module docstring declares it **"stdlib-only and intentionally self-contained"** — a documented architectural invariant that predates this rethink and that reversing this direction would break (it would need to import `renmark.schemas`). This is a genuine conflict between the roadmap's generic migration note and an existing, deliberate design decision, not a mechanical follow-on — left untouched pending an explicit Owner call on whether the stdlib-only guarantee for `delivery_state.py` still holds or should be relaxed.
**Files changed:** `renmark/schemas.py`, `renmark/lifecycle.py`, `renmark/dispatch.py`.
**Do not change:** `renmark/delivery_state.py` stays stdlib-only until the Owner explicitly decides otherwise — do not import `renmark.schemas` into it as a "finish the job" follow-up without that decision. None of the three separately Owner-protected areas (role model/ADR-001, `AskUserQuestion` selector, task tracking) were touched. Full suite: 1936 passed, 31 skipped — unchanged from before this refactor (pure import-direction change, no behavior change).

## [2026-08-04] — chore: rethink Release 2 — remove context_budget_hint dead code (REQ-5)
**Request:** Owner said "continue next" after Release 1 — next item on the Owner-approved rethink roadmap (`.renmark/rethink/renmark-architecture/roadmap.md`) is Release 2, removing `renmark.state.skills.context_budget_hint` (classification.md item 7: dead scaffolding with zero production callers, re-verified by repo-wide grep before and after this change).
**Built:** Drafted a 5-task plan (`.renmark/plans/2026-08-04-rethink-release-2-remove-context-budget-hint.plan.md`) for `/renmark:orchestrate`, but the subagent gate (`renmark.subagent_gate`) correctly challenged all 5 as unjustified — simple grep-verifiable deletions across known files are inline-able, not subagent-dispatch-worthy. Made the edits directly instead: removed `context_budget_hint` + its three threshold constants from `renmark/state/skills.py` (the module's other functions — `record_skill_invocation`, `context_budget_check`, `last_skill_invocation` — are still called elsewhere and are untouched); deleted `tests/test_state_skills.py` (its only test covered the now-removed function); updated `CLAUDE.md`'s and `AGENTS.md`'s mirrored `context-thresholds-rule` paragraphs and `plugin/skills/.shared/agency-delivery.md`'s two references to describe the 100k/120k/150k tiers as self-monitored prose thresholds rather than pointing at a deleted function — the enforced behavior is unchanged, only the dead pointer is gone.
**Files changed:** `renmark/state/skills.py`, `tests/test_state_skills.py` (deleted), `CLAUDE.md`, `AGENTS.md`, `plugin/skills/.shared/agency-delivery.md`.
**Do not change:** None of the three Owner-protected areas (role model/ADR-001, `AskUserQuestion` selector, task tracking) were touched — verified via diff before commit. Full suite: 1936 passed, 31 skipped (down 1 test from the deleted file, as expected; zero new skips vs. baseline).

## [2026-08-03] — test: rethink Release 1 — baseline/compatibility coverage (REQ-30)
**Request:** `/renmark:rethink` on renmark itself produced an Owner-approved 7-release roadmap (`.renmark/rethink/renmark-architecture/roadmap.md`); Owner approved execution via the Stage 9 Execution Gate and directed Release 1 to dispatch via `/renmark:orchestrate`, with explicit protection for three items: the Owner/General-Contractor/Architect/Worker/Inspector role model (ADR-001), the `AskUserQuestion` selector contract, and native task tracking (REQ-31).
**Built:** `tests/test_artifact_home_and_baseline_compat.py` — turns `baseline.md`'s point-in-time claims about the artifact-home convention (specs/plans/reviews/state/memory canonical homes) and repo-root discovery into runnable pytest coverage, additive to the existing STAGES-order and host-capability-table tests. Test-only; no production code moved. Release 1's second roadmap item (`r1-req30-baseline-measurement` — live numeric measurement across Start/Feature-Fix/Orchestrate/Rethink) was deliberately excluded from this dispatch: it requires full end-to-end pipeline runs, a materially larger spend than a bounded file task, and stays `pending` in `.renmark/state/program.json` until explicitly scheduled.
**Files changed:** `tests/test_artifact_home_and_baseline_compat.py` (new).
**Do not change:** None of the three Owner-protected items (`renmark/agency.py`/ADR-001 role model, `interaction-contract.md`/`build_selector`, `task_tracking.py`/`task-tracking.md`) were touched by this task — verified via diff before commit. Full suite: 1937 passed, 31 skipped (baseline was 1931/31 — zero new skips).

## [2026-08-02] — feat: orchestration baseline controls — usage instrumentation, context checkpoint, routing enforcement, artifact lifecycle (REQ-30/REQ-31)

**Request:** Owner-accepted ORCHESTRATION-BASELINE-2026-08 audit (`.renmark/audits/orchestration-baseline-audit-2026-08-02.md`) identified five real gaps: per-dispatch token/usage telemetry hardcoded to 0, `context_budget_hint` dead code with zero production callers, the model-routing table enforced only at a `--dry-run` preview surface (never real dispatch), no artifact lifecycle/retirement policy, and 32+ stale/duplicate artifacts. Owner approved implementing bounded controls for all five without changing pipeline outcomes, Owner gates, verification strength, recovery, or Agency/milestone behavior — extending existing REQ-30/REQ-31 machinery, no parallel systems.
**Built:** Two plan files (`.renmark/plans/2026-08-02-orchestration-baseline-controls-part{1,2}.plan.md`, 18 tasks total) plus a 3-task codereview fix pass:
- **Usage instrumentation:** `UsageRecord.measured: bool` (new field, `renmark/state/usage.py`); `record_task_run(..., measured=False)` + honest `measured_tokens_total`/`unmeasured_task_count` aggregation in `renmark/analytics.py`'s `_agg_tasks` (existing `tokens_by_executor`/`tokens_by_model`/`tokens_by_provider` keys untouched — additive only). New `plugin/skills/.shared/usage-instrumentation.md` — a two-layer live-host-vs-headless contract mirroring REQ-31's task-tracking pattern: the live agent calls `record_task_run(measured=True)` with real Agent-tool usage numbers after each dispatch; the headless Codex path stays honestly `measured=False` (Codex CLI doesn't surface usage — disclosed, not a bug). Cited from `orchestrate/SKILL.md`.
- **Context-lifecycle checkpoint:** `renmark.lifecycle.milestone_context_checkpoint(repo, skill, estimated_tokens=None, host=None)` — returns `None` with no signal or below `config.compact_gate_tokens`'s threshold (never compacts on every milestone); at/above threshold, persists a checkpoint and returns a manual `/compact` + `/renmark:resume` instruction (no SDK exists to send `/compact` programmatically — confirmed by the audit; never fabricated automation). Wired best-effort into `renmark/agency.py`'s `approve_milestone_for_orchestrator` at the real milestone-boundary event (`kind="agency-approved-handoff"`). **Known limitation** (flagged by codex codereview as spec "under-built," documented in `.renmark/memory/orchestration-baseline.md`): the wiring always passes `estimated_tokens=None` today — no reliable Python-side context-size signal exists yet, so the hook is dormant, not an active trigger, pending either a host-exposed context-size API or a self-reported estimate.
- **Routing enforcement:** `renmark/plan_lint.py` Check 12 (`escalation_reason_for` / `_check_escalation_justified`) — WARN-only (never BLOCK) when an `opus` task lacks a `requires_escalation()`-satisfying justification, enforced at the real `check-plan` pre-dispatch gate (not just `--dry-run` preview). Deliberately scoped to `opus` only, not `fable` (fable already has dedicated BLOCK checks 9/10). `renmark.memory.append_routing` gained an `escalation_reason` kwarg; wired into the headless `execute_plan` routing log (`renmark/cli/_engine.py`'s `_memory_log_outcome`).
- **Artifact lifecycle:** new `plugin/skills/.shared/artifact-lifecycle.md` (owner/status/dependencies/invalidated_by/replacement/retention rules, additive to CLAUDE.md's existing provenance block) plus a formalized independent-reviewer risk-trigger checklist in `subagent-budget.md`. Two targeted retirements per the audit: `.renmark/roadmap/agency-optimization-roadmap.md` retired to a pointer stub (not deleted — backward-compatible read) pointing at the canonical `.renmark/memory/roadmap.md`.
- **Codereview fixes (3):** false-positive architecture WARN when an `opus` task's complexity wasn't `hard` (Check 12 now derives `kind` from the spec-text signal independent of complexity); zero-token rows undercounted in the new honesty metrics (`_agg_tasks` now classifies every row's measured status unconditionally); documentation clarifying the dormant-hook status above.
- **Two unrelated pre-existing bugs found and fixed live, separate from the above:** `renmark/delivery_state.py`'s `append_provenance_event` only capped provenance history by event *count* (24), not byte size — normal accumulated milestone history (long `detail` strings from recent R-0.2/R-0.3 milestones) had already pushed `delivery.json` past its 4096-byte budget, crashing every `renmark-execute` invocation before this branch started. Added byte-budget-aware trimming (drops oldest events until it fits, mirrors the count-cap's keep-most-recent semantics). `renmark/cli/_engine.py`'s `_complete_clean_run` plain-wrote `work_packages` with no archival step — the same bug class surfaced a second time live during this session's own dispatches; now calls the existing `archive_completed_work_packages` instead.
**Files changed:** `renmark/state/usage.py`, `renmark/analytics.py`, `renmark/lifecycle.py`, `renmark/agency.py`, `renmark/plan_lint.py`, `renmark/memory.py`, `renmark/cli/_engine.py`, `renmark/delivery_state.py`, `plugin/skills/.shared/usage-instrumentation.md` (new), `plugin/skills/.shared/artifact-lifecycle.md` (new), `plugin/skills/.shared/subagent-budget.md`, `plugin/skills/.shared/model-routing.md`, `plugin/skills/orchestrate/SKILL.md`, `.renmark/memory/orchestration-baseline.md`, `.renmark/roadmap/agency-optimization-roadmap.md`, `tests/test_usage_instrumentation.py` (new), `tests/test_context_checkpoint.py` (new), `tests/test_plan_lint_escalation.py` (new), `tests/test_delivery_state.py`, `tests/test_state_pause_usage.py`, `tests/test_plan_lint.py`. Full suite: 1931 passed, 31 skipped.
**Do not change:** `milestone_context_checkpoint` must not be fed a fabricated `estimated_tokens` — that would violate "never compact after every milestone"; a real signal source (host API or self-reported estimate) must exist first. Check 12 stays WARN-only and opus-scoped — do not extend it to BLOCK or to fable without revisiting the fable-fixture conflict this branch resolved by narrowing scope. The delivery-state byte-budget trim drops oldest provenance events silently (recoverable from CHANGELOG.md/`.renmark/reviews/`) — do not tighten to fail-loud without deciding that tradeoff explicitly, mirroring R-0.3's existing best-effort-ledger precedent. PRD.md REQ-30/REQ-31 amendments reflecting this work were deliberately NOT made here (only `/renmark:prd` writes `PRD.md`) — noted as a pending follow-up, not done.

## [2026-08-02] — feat: native task tracking for dispatched work (REQ-31)

**Request:** Owner-directed: add native task tracking to every renmark pipeline whenever work is dispatched, implemented once in the shared dispatch contract rather than duplicated per pipeline; two Stop-hook rejections during this work required corrections (documentation-only enforcement, then conflating the live-host-tool mechanism with the Python CLI mirror).
**Built:** New shared fragment `plugin/skills/.shared/task-tracking.md` — the single source of truth for native task-tracking mechanics, wired into every dispatching pipeline via the existing `subagent-budget.md` dispatch-packet contract (no per-pipeline duplication). New PRD `REQ-31`, naming **two distinct, non-substitutable mechanisms**: (i) the live host's own `TaskCreate`/`TaskUpdate` — the only thing that satisfies "create a native task using the available Task tools," callable only by the agent actually executing a skill in an interactive session; and (ii) `renmark.task_tracking` (new module, `.renmark/state/tasks.json`, atomic writes, never-raising reads), a real Python mirror wired into the one dispatch path with no live session at all — `renmark.cli._engine.execute_plan`'s `_runner` (the headless `codex`/subprocess executor). Mechanism (ii) creates one parent task per plan run, one worker task per dispatched task, and one linked verification task per worker; `complete_worker_task` mechanically enforces no-self-approval by reusing `renmark.ledger.check_dispatch_independence` (the same dispatch-identity check R-0.4's Inspector already relies on) rather than trusting a caller-asserted flag. Demonstrated mechanism (i) live in-session: a real `TaskCreate` → `TaskUpdate(in_progress)` → `Agent` dispatch → `TaskUpdate(completed, with evidence)` sequence, using this session's actual native tools, verifying the final diff (marker balance, no leftover `.tmp.*` files, `py_compile` clean).
**Files changed:** `plugin/skills/.shared/task-tracking.md` (new), `plugin/skills/.shared/subagent-budget.md`, `plugin/skills/orchestrate/SKILL.md`, `renmark/task_tracking.py` (new), `renmark/cli/_engine.py`, `PRD.md` (REQ-31 + two revision notes documenting the corrections + Goals bullet), `CLAUDE.md`/`AGENTS.md` (new `task-tracking-rule` block, mirrored), `tests/test_task_tracking.py` (new, 15 unit tests), `tests/test_task_tracking_engine_wiring.py` (new, 5 real-dispatch-loop integration tests), `tests/test_task_tracking_contract.py` (new, 25 contract tests).
**Do not change:** `renmark.task_tracking` is scoped strictly to the headless `execute_plan` path — it must never be presented as satisfying requirement 1 for a live interactive session; that is always the live host's own `TaskCreate`/`TaskUpdate`. Every `task_tracking` call site in `_engine.py` is best-effort/never-raising (matches the existing ledger-emission convention) — do not make it raise into the real dispatch/commit path. `complete_worker_task`'s no-self-approval check must keep reusing `renmark.ledger.check_dispatch_independence`, not a reimplementation.

## [2026-08-02] — feat: orchestration efficiency and UX stability is a protected capability (REQ-30)

**Request:** Owner-directed: analyze the current PRD and add a cross-cutting requirement protecting renmark's low-token, low-latency orchestration behavior from future regression, with a named, measurable baseline rather than a subjective "still feels fast" impression.
**Built:** New PRD `REQ-30`, citing rather than restating REQ-2 (model routing), REQ-5 (context hygiene), REQ-20 (context taxonomy), REQ-21 (deterministic-first), REQ-22 (milestone execution), REQ-24 (recurrence/anti-duplicate-dispatch), and REQ-27 (work classification/fast path) — declaring their aggregate behavior a protected capability with a quantified regression-block rule (>15% median token/time increase, an added routine gate, a duplicate dispatch, worker-context leakage into the orchestrator, or weakened verification/recovery blocks a release absent an explicit, evidence-backed Owner exception with a rollback path). Named baseline **`ORCHESTRATION-BASELINE-2026-08`**, pinned to `v0.39.7` (commit `d9cccc5`), recorded at `.renmark/memory/orchestration-baseline.md` — the qualitative guarantees and the four representative benchmark scenarios (Start/Feature-Fix/Orchestrate/Rethink) are defined there; actual token/latency numbers are explicitly left uncaptured pending a real benchmark run (never fabricated). Added a one-sentence REQ-30 citation to all 8 dispatching pipeline surfaces.
**Files changed:** `PRD.md` (REQ-30 + Goals bullet + revision note), `.renmark/memory/orchestration-baseline.md` (new), `.renmark/memory/INDEX.md`, `CLAUDE.md`/`AGENTS.md` (new `orchestration-efficiency-rule` block, mirrored), `plugin/skills/{init,start,feature,debug,roadmap,finish,rethink,orchestrate}/SKILL.md` (one-sentence citation each), `tests/test_orchestration_efficiency_requirement.py` (new, 19 tests).
**Do not change:** The baseline artifact's honest "not yet measured" status must not be silently upgraded with invented numbers — a real benchmark run is required first. The 15% regression threshold and the exception's evidence/approval/rollback requirements are the enforcement mechanism; do not weaken them to accommodate a convenient feature.

## [2026-08-02] — feat: evidence-based greenfield entry point for /renmark:start (REQ-29)

**Request:** Owner-directed, in three rounds: (1) give `/renmark:start` the same evidence/traceability/modularity discipline `/renmark:rethink` has, adapted to greenfield (no brownfield survey/baseline/classification); (2) restructure the single combined gate into three named Owner gates matching rethink's pattern; (3) no further correction needed for start this round.
**Built:** New PRD `REQ-29`. For **Complex** scope builds, four new mandatory stages threaded into the existing pipeline: external research (Step 4.5, bounded `researcher` subagent, honest `blocked`/`incomplete` reporting — never backed by model memory), the **Discovery Direction Gate** (Step 4.6 — findings/implications/recommended direction/alternatives/risks/exact decision, replacing the plain Step-5 confirmation for Complex scope), a PRD acceptance/traceability contract (Step 5a-ii), a prospective modular blueprint (Step 5b, now mandatory not merely offered), the **Solution Gate** (Step 5c — scope/workflows/requirements/module boundaries/exclusions/tradeoffs, before roadmap finalization), and the **Execution Gate** (Step 7a — the finalized roadmap/plan, before any target code). A cross-cutting **exception check-in** interrupts immediately on a material conflict, unreliable research, a major cost/scope/security impact, or a high-impact unknown. **Simple** scope collapses all of this into one documented, Owner-approved waiver inside the existing single confirmation — the entry-question-plus-2-follow-ups fast path is unchanged and no additional gate is added.
**Files changed:** `plugin/skills/start/SKILL.md`, `PRD.md` (REQ-29 + Goals bullet + two revision notes), `plugin/skills/help/SKILL.md`, `CLAUDE.md`/`AGENTS.md` (routing-rule sentence), `tests/test_start_pipeline.py` (new, 27 tests).
**Do not change:** The Simple-scope fast path (one open question, ≤2 follow-ups, one confirmation) must never grow a second gate — the waiver folds into the existing confirmation, it does not replace it with ceremony. The Discovery Direction Gate replaces, never supplements, Step 5's plain confirmation for Complex scope — never present both.

## [2026-08-02] — feat: evidence-based modernization pipeline for /renmark:rethink (REQ-28 expansion)

**Request:** Owner-directed, in three rounds: (1) upgrade `/renmark:rethink` from a six-stage brownfield survey into a nine-stage evidence-based pipeline (external benchmarking distinct from internal survey, a binding PRD acceptance contract, a mandatory modularity assessment); (2) restructure the single Owner gate into three named gates (Discovery Direction, Solution, Execution) plus a cross-cutting exception check-in, mirroring the same pattern later applied to `/renmark:start`.
**Built:** Expanded `REQ-28` (not a new requirement — same brownfield discipline, elaborated). Nine bounded stages: internal survey → behavioral baseline → PRD acceptance contract → external discovery/benchmarking (separate from the internal survey, honest `blocked`/`incomplete` reporting) → modularity/scalability assessment → evidence-based Keep/Improve/Replace/Remove/Unknown-Spike classification (citing internal + PRD + external + modularity evidence; `Improve` items may get architectural boundary changes, not only `Replace`) → target modular blueprint → incremental transformation roadmap (`renmark.program`, first release defaults to baseline/compatibility coverage) → Owner gate + hand-off to existing Agency/milestone machinery. Added a **Transformation Intake** (Step 0a) before stage 1, the **Discovery Direction Gate** (stage 5a, between modularity assessment and classification), the **Solution Gate** (stage 7a, between blueprint and roadmap), and elaborated stage 9 into the explicit **Execution Gate**. Cross-cutting **exception check-in** for material conflicts, unreliable research, cost/scope/security impact, proposed removal of protected behavior, or an unboundable high-impact unknown.
**Files changed:** `plugin/skills/rethink/SKILL.md`, `plugin/commands/rethink.md`, `PRD.md` (REQ-28 + Goals bullet + two revision notes), `plugin/skills/help/SKILL.md`, `CLAUDE.md`/`AGENTS.md` (routing-rule sentence), `tests/test_rethink_pipeline.py` (new, 36 tests).
**Do not change:** Rethink still hands off to renmark's existing Agency/milestone execution machinery — never a rethink-only executor. The first roadmap release still defaults to baseline/compatibility coverage unless the Owner explicitly overrides with recorded evidence. No stage before the Execution Gate may touch the target application's production code.

## [2026-08-02] — feat: add /renmark:rethink brownfield transformation pipeline

**Request:** Implement REQ-28 — a new `/renmark:rethink` pipeline for reassessing and transforming an existing application, distinct from `/renmark:start`'s greenfield lane, per `.renmark/plans/2026-08-02-add-rethink-pipeline-skill.plan.md`.
**Built:** `plugin/skills/rethink/SKILL.md` (new) — six bounded stages (survey → behavioral baseline → Keep/Improve/Replace/Remove/Unknown-spike classification → target blueprint → transformation roadmap as a `renmark.program` → Owner gate + hand-off to renmark's existing Agency/milestone execution machinery, reusing `agency.activate`/`program.write_program` rather than a bespoke executor). `plugin/commands/rethink.md` (new) — thin command wrapper. Updated every place renmark documents its pipeline list to seven pipelines: `plugin/.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json` descriptions; `CLAUDE.md`/`AGENTS.md` routing sentences (mirrored); `README.md` (bullet list, pipeline table, commands table); `plugin/skills/help/SKILL.md` (pipeline block, quick map, skills list); `plugin/skills/guide/SKILL.md` (routing question + answer table, renumbered options 3-9). Pre-dispatch gate (`renmark.subagent_gate`) correctly flagged 9 of 10 tasks as inline-able mechanical edits (only the SKILL.md itself warranted a subagent) — done inline per that gate's recommendation, one opus subagent for the SKILL.md only. Registered the new skill in `renmark/skillmeta.py` (`SKILLS["rethink"]`), `renmark/lifecycle.py` (`IMPLEMENTED_SKILLS`, `PIPELINE_SKILLS`) — required beyond the original plan for the skill to not be an audit "ghost"/registry gap; caught by a full-suite regression run (`test_audit.py`, `test_lifecycle.py`, `test_skillmeta.py`), not anticipated in the plan. Full suite: 1782 passed, 31 skipped, 0 failed.
**Files changed:** `plugin/skills/rethink/SKILL.md` (new), `plugin/commands/rethink.md` (new), `plugin/.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `CLAUDE.md`, `AGENTS.md`, `README.md`, `plugin/skills/help/SKILL.md`, `plugin/skills/guide/SKILL.md`, `renmark/skillmeta.py`, `renmark/lifecycle.py`, `.renmark/plans/2026-08-02-add-rethink-pipeline-skill.plan.md` (new).
**Do not change:** `plugin/.codex-plugin/plugin.json` was deliberately left untouched — its description is generic and names no per-pipeline count, so there was nothing to update there. Rethink hands execution off to renmark's existing `agency.py`/`program.py`/`orchestrate`/`finish` machinery — it must not grow a parallel executor. The first release in any rethink-produced roadmap defaults to baseline/compatibility coverage, not architecture replacement, per REQ-28's acceptance criterion — do not silently reorder that default.
...
**Built:** WP-1 (`plugin/agents/inspector.md` — read-only Inspector role; design doc naming the exact `InspectionReport` fields and the mechanical independence signal). WP-2 (`dispatch_identity` field added to `WorkResult`/`InspectionReport`; `check_dispatch_independence()` + `DispatchIndependenceError`, rejecting same-identity or empty-inspector-identity verdict attempts before they reach the ledger; empty-worker-identity alone tolerated for legacy R-0.3 events). WP-3 (`emit_inspection_verdict()` — runs the independence check first, then constructs and appends an `InspectionReport`; `latest_verdict_for()` reader). WP-4 (wired one real call site: `execute_plan`'s `_runner` in `renmark/cli/_engine.py`). WP-5 (9 new tests across `tests/test_ledger.py`/`tests/test_ledger_wiring.py`; also fixed a real regression WP-1 introduced — `inspector.md` had no matching entry in `subagent_profiles.PROFILES`/`_NATIVE_AGENT_ROLES`, registered as the project's 9th specialized dispatch role). WP-6's first independent review returned **ESCALATE**: the wired call site relabeled the Worker's own verifier boolean under a different identity string rather than performing an independent judgment, contradicting the contract's "separate Agent/executor call" wording (F-R0.4-1). Owner-scoped bounded repair, WP-4b: reject the mandatory-LLM-dispatch alternative; instead rerun the task's verifier as a genuinely separate, fresh subprocess execution and derive the verdict/evidence from that rerun, not the Worker's `ok`. Contract wording amended to "a separate inspection execution" — a deterministic independent-verifier rerun is the default Inspector for this minimal release; the LLM Inspector role stays optional. WP-6b re-review (fresh, independent) returned **PASS**, confirming F-R0.4-1 closed. New non-blocking follow-up, **F-R0.4-2**: the verifier rerun is read-only by convention/sampling, not mechanically enforced — deferred to Milestone 5's Inspector registry work. Full suite: 1776 passed, 31 skipped, 0 failed.
**Files changed:** `renmark/ledger.py`, `renmark/cli/_engine.py`, `renmark/subagent_profiles.py`, `plugin/agents/inspector.md` (new), `tests/test_ledger.py`, `tests/test_ledger_wiring.py`, `tests/test_subagent_profiles.py`, `.renmark/plans/2026-08-01-r-0.4-minimal-independent-inspector-contract.md` (new), `.renmark/plans/2026-08-01-r-0.4-wp1-inspector-design.md` (new), `.renmark/reviews/2026-08-01-r-0.4-wp6-independent-review.md` (new), `.renmark/reviews/2026-08-01-r-0.4-wp6b-rereview.md` (new), `CLAUDE.md`, `AGENTS.md`.
**Do not change:** `inspector` is read-only (no Write/Edit) and MUST only emit verdicts via `ledger.emit_inspection_verdict` — it does not touch production files. The Inspector's verifier rerun assumes verifier commands are side-effect-free (pytest/py_compile/lint-style checks); this is convention, not enforced (F-R0.4-2) — do not wire a mutating verifier command through this path without addressing that gap first. `dispatch_identity` is optional on `WorkResult`/`InspectionReport` for legacy-event compatibility — do not make it required without a migration plan for pre-R-0.4 ledger events. Version bump / tag / RELEASED status is a separate Owner gate, not yet exercised as of this entry.

## [2026-08-01] - release(R-0.3): minimal canonical ledger — ACCEPTED, no follow-up

**Request:** Start R-0.3 (Minimal Canonical Ledger), scoped narrowly to Work Order/Work Result/Inspection Report/Escalation only. Carry R-0.2's F1/F2/F4/F5 as tracked dependencies without letting them expand scope unless they block the ledger. Continue through routine work packages without re-approval; stop only for a concrete blocker.
**Built:** WP-1–WP-3 (`renmark/ledger.py` — four dataclasses, per-kind validation, `append_ledger_event`/`read_ledger_events` against `.renmark/ledger/events.jsonl`, reject-before-write so a malformed event never corrupts the ledger). WP-4 (one real emission call site per kind: `execute_plan`'s per-task dispatch/return in `renmark/cli/_engine.py`, `write_report` in `renmark/scan.py`, `write_pause` in `renmark/state/pause.py` — each best-effort/never-raising). WP-5 (21 tests total across `tests/test_ledger.py` and `tests/test_ledger_wiring.py`, folded into WP-1–4 since coverage was already complete). WP-6 (independent review — PASS, no follow-ups, `.renmark/reviews/2026-08-01-r-0.3-wp1-4-review.md`, including a scratch-run acceptance-evidence demonstration that the ledger read-back alone reconstructs "what was ordered, what came back, what was inspected, what was escalated"). Closeout at `.renmark/reviews/2026-08-01-r-0.3-closeout.md`: **ACCEPTED, no follow-up** — the first Renmark milestone to close clean on the first pass, a direct result of the Owner's explicit scope discipline keeping F1/F2/F4/F5 tracked-only. `/renmark:resume` also found `delivery.json`/lifecycle state stale (still showing R-0.2 as active despite its release) and corrected it as bookkeeping, not a blocker.
**Files changed:** `renmark/ledger.py` (new), `tests/test_ledger.py` (new), `tests/test_ledger_wiring.py` (new), `renmark/cli/_engine.py`, `renmark/scan.py`, `renmark/state/pause.py`, `tests/test_scan.py` (fixed a pinned `now_iso()` call-count test broken by the new emission call), `.renmark/plans/2026-08-01-r-0.3-minimal-canonical-ledger-contract.md` (new), `.renmark/reviews/2026-08-01-r-0.3-wp1-4-review.md` (new), `.renmark/reviews/2026-08-01-r-0.3-closeout.md` (new), `.bootstrap-renmark/milestones/INDEX.md`, `VERSION`, `pyproject.toml`, `renmark/__init__.py`.
**Do not change:** Ledger emission is intentionally best-effort (`try/except: pass`) at all four call sites — a ledger bug must never block the real report/dispatch/pause it's attached to; don't tighten to fail-loud without deciding that tradeoff explicitly. `WorkOrder.is_repair`/`repairs_finding_ref` and `Escalation.is_replannable`/`replan_evidence` are schema-readiness fields only (F2/F5) — no live caller constructs either variant; do not cite their existence as evidence F2/F5 are closed. Only `execute_plan`'s real (non-dry-run) path emits Work Order/Result today — other dispatch routes (fast-path, `/renmark:feature`, `/renmark:debug`, `/renmark:orchestrate`) do not yet, and that gap is explicitly out of scope per this contract's "universal rewiring" exclusion, not an oversight.

## [2026-08-01] - R-0.3 WP-4: wire real ledger emission call sites

**Request:** Continuing R-0.3 under Continue-by-Default — wire one real emission point per artifact kind (bounded proof, not universal rollout, matching R-0.1/R-0.2 precedent).
**Built:** `renmark/cli/_engine.py`'s `execute_plan` now emits a `WorkOrder` right before each task's real dispatch and a matching `WorkResult` on return (order_id `{run_id}-{task_index}`), both best-effort/never-raising. `renmark/scan.py`'s `write_report` emits an `InspectionReport` (verdict derived from finding count) after every scan report write. `renmark/state/pause.py`'s `write_pause` emits an `Escalation` — since it's the single canonical writer behind every PAUSED-state call site (usage-limit pauses, manual pauses, wave-dispatch failures), this one wiring point covers all of them. Fixed a pre-existing `test_scan.py` test that pinned an exact `now_iso()` call count via a finite monkeypatched iterator — the new `InspectionReport` emission adds one more call per `write_report`, so extended the stamp sequence by 2. Added `tests/test_ledger_wiring.py` (5 tests) proving each call site appends a well-formed event offline, no live Agent/LLM/codex call (`_execute_task` monkeypatched to a canned result). Full suite: 1767 passed, 31 skipped.
**Files changed:** `renmark/cli/_engine.py`, `renmark/scan.py`, `renmark/state/pause.py`, `tests/test_ledger_wiring.py` (new), `tests/test_scan.py`, `.renmark/plans/2026-08-01-r-0.3-minimal-canonical-ledger-contract.md`.
**Do not change:** These are the ONE real call site per kind per this milestone's bounded-proof scope — do not read this as "every dispatch path now emits to the ledger" (only `execute_plan`'s real-run path does; `dry_run` and any other dispatch route do not yet). Ledger emission is intentionally best-effort (`try/except: pass`) at every call site — a ledger bug must never block the real report/dispatch/pause it's attached to; do not tighten this to fail-loud without deciding that tradeoff explicitly first.

## [2026-08-01] - R-0.3 WP-1–3: minimal canonical ledger (schema + writer/reader)

**Request:** Start R-0.3 (Minimal Canonical Ledger), scoped narrowly to consolidating four core artifacts — Work Order, Work Result, Inspection Report, Escalation — into one schema-validated append-only ledger. R-0.2's F1/F2/F4/F5 follow-ups carried as tracked dependencies, not expanded scope, per Owner instruction. `/renmark:resume` found `delivery.json`/lifecycle still showing R-0.2 as active despite it being RELEASED (v0.39.5) — corrected as stale bookkeeping, not a blocker. Contract drafted, reviewed in full, and approved as drafted (Owner, AskUserQuestion).
**Built:** WP-1 (schema design) + WP-2 (ledger writer) + WP-3 (reader) in one bounded pass: `renmark/ledger.py` — `WorkOrder`, `WorkResult`, `InspectionReport`, `Escalation` dataclasses, per-kind validation (no external jsonschema dep — none declared in `pyproject.toml`), `LedgerValidationError` on malformed writes (no silent corruption), `append_ledger_event()` / `read_ledger_events()` against `.renmark/ledger/events.jsonl`. `WorkOrder` carries an unwired `is_repair`/`repairs_finding_ref` (F2 schema-readiness) and `Escalation` an unwired `is_replannable`/`replan_evidence` (F5 schema-readiness) — schema shape only, no production caller, matching the contract's "carried, not re-opened" instruction. 16 new tests in `tests/test_ledger.py`, all passing; ran alongside `test_program.py`/`test_lifecycle.py` (135/135) to confirm no import-time breakage.
**Files changed:** `renmark/ledger.py` (new), `tests/test_ledger.py` (new), `.renmark/plans/2026-08-01-r-0.3-minimal-canonical-ledger-contract.md` (new), `.bootstrap-renmark/milestones/INDEX.md` (R-0.3 row added).
**Do not change:** WP-4 (wiring a real emission call site per artifact kind) is not yet done — nothing in the live pipeline emits to the ledger yet, this pass only built the primitives. Do not treat `is_repair`/`is_replannable` fields as evidence that F2/F5 are closed — they are schema readiness only, explicitly out of scope for R-0.3 per its contract's "Carried dependencies" section. `.renmark/state/delivery.json` is gitignored runtime state — it was updated locally (active milestone → R-0.3, R-0.2 marked released) but does not appear in this commit.

## [2026-08-01] - release(R-0.2): controlled worker execution — ACCEPTED WITH FOLLOW-UP

**Request:** Draft and approve R-0.2 (Controlled Worker Execution: the remainder of Phase 2 not covered by R-0.1's fast path — generalize Worker-scope enforcement, enforce Inspectors-cannot-repair, a dispatch reason/budget gate, evidence-required replanning, a uniform rework cap), R-0.1's gate. GitHub verification treated as unrelated, non-blocking. Owner instruction: progress work packages without stopping for permission between each one, matching R-0.1's precedent — codified in this release as the durable "Renmark Continue-by-Default Policy" in `CLAUDE.md`/`AGENTS.md`.
**Built:** WP-1–WP-3 + WP-6 (design docs for scope-enforcement generalization, Inspector/repair separation + ADR-046, dispatch-reason/budget gate, evidence-required replan, nested-dispatch signal investigation — found the 8 native `plugin/agents/*.md` roles already structurally block nested dispatch via tool-allowlist exclusion, `general-purpose`/Codex remain an open gap). WP-4 pinned a pre-implementation regression baseline (8 tests). WP-5 implemented all five mechanisms in parallel on disjoint files: `AgentDispatch.scope`/`verify_agent_dispatch_scope()` in `dispatch.py`; `R008Checklist`/`enforce_r008_dispatch()` in `subagent_gate.py`; `Write` dropped from `reviewer.md`'s tool allowlist plus `RepairWorkOrder`/`build_repair_work_order()`; `permit_replan()`/`ReplanDecision` in `program_driver.py` reusing the existing recurrence ledger. WP-7's independent review found every mechanism correct-and-tested but with zero production callers (5/8 acceptance criteria PARTIAL, a materially larger gap than R-0.1's one). Owner chose to hold the release and add WP-8, a bounded wiring pass (5 sub-tasks) that wired real call sites in `dispatch.py`'s wave dispatcher, `cli/_engine.py`, `delivery_state.py`, and `SKILL.md` prose in `codereview`/`feature`/`start`/`debug`. A WP-8 re-review found the gap narrowed but not closed; Owner authorized one final bounded WP-9 pass on the remaining scope-enforcement gap (`enforce_wave_dispatch_scopes()`, fail-loud), which closed the "silently ignorable" half but not the "no production caller" half (the async dispatch path has no synchronous post-call git state to check, same limitation `fast_path.py` already documents). Final acceptance: **ACCEPTED WITH FOLLOW-UP**, explicitly weaker than R-0.0/R-0.1's outcome — see `.renmark/reviews/2026-08-01-r-0.2-closeout.md` for the full criterion-by-criterion record.
**Files changed:** `renmark/dispatch.py`, `renmark/subagent_gate.py`, `renmark/program_driver.py`, `renmark/delivery_state.py`, `renmark/cli/_engine.py`, `renmark/providers/claude_agent.py`, `plugin/agents/reviewer.md`, `plugin/skills/{codereview,feature,start,debug}/SKILL.md`, `.renmark/plans/2026-08-01-r-0.2-controlled-worker-execution-contract.md` (new — canonical `.renmark/plans/`, not `.bootstrap-renmark/milestones/`, per Owner instruction), `.renmark/plans/r-0.2/*.md` (5 design docs, new), `.renmark/reviews/2026-08-01-r-0.2-closeout.md` + `r-0.2-comparison.md` (new), `.renmark/memory/decisions.md` (ADR-046), `CLAUDE.md`/`AGENTS.md` (Continue-by-Default Policy), `.bootstrap-renmark/milestones/INDEX.md`, 9 new `tests/test_{dispatch_scope_generalization,r0_2_dispatch_regression_baseline,subagent_gate_r008,repair_work_order,replan_evidence_gate,wp8_scope_wiring,wp8_r008_wiring,wp8_repair_wiring,wp9_scope_enforcement_wiring}.py`, `VERSION`, `pyproject.toml`, `renmark/__init__.py`, `plugin/.claude-plugin/plugin.json`, `plugin/.codex-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `README.md`, `CHANGELOG.md`.
**Do not change:** Do not assume R-0.2's enforcement mechanisms are fully live in production — `enforce_wave_dispatch_scopes()`, `enforce_r008_dispatch()` (strict mode), and `permit_replan()` exist, are tested, and are only partially or lenient-ly wired (F1/F3 residual); the repair-work-order pattern and rework-cap uniformity are prose-invoked only outside `orchestrate`/`codereview` (F2/F4/F5) — a future release must wire these further before citing this release's "expected result" language as literally true today. `build_agent_dispatch`'s default (unscoped) behavior must stay unchanged for backward compatibility. `ruff`/`mypy` remain unavailable in this environment (F7) — `pytest` was the only dev gate actually exercised.

## [2026-08-01] - release(R-0.1): bounded small-task fast path — ACCEPTED WITH FOLLOW-UP

**Request:** Build R-0.1 (Bounded Small-Task Fast Path: Phase 1 UX/compatibility harness + the blocking subset of Phase 2 authority boundaries), the release R-0.0 gates. All 5 work packages (classification design, Worker scope-enforcement design + ADR-002, UX regression baseline, implementation, benchmark comparison) run in sequence per Owner instruction, without re-gating between routine WPs.
**Built:** `renmark/fast_path.py` (new) — `classify_fast_path()`, the 5-signal deterministic fast-path eligibility check, and `WorkerScope`/`verify_worker_scope()`, a deterministic post-execution check comparing a Worker's declared scope against its ACTUAL `git diff --name-status`, never self-reported `touched_files` — the direct answer to R-0.0's Scenario C unauthorized-delete finding. `renmark/providers/claude_agent.py` gained `build_fast_path_agent_dispatch()` (new, additive — the existing `build_agent_dispatch` is untouched and independently verified as such) composing a real dispatch prompt with explicit scope, explicit delete/rename prohibition, and explicit (contractual, not mechanically enforced — flagged, not hidden) no-nested-dispatch prohibition. 20 new tests including the literal required reproduction of R-0.0's Scenario C shape (a scope excluding 4 pre-existing files, a diff deleting them, asserted to fail). A live subagent benchmark run (`.bootstrap-renmark/metrics/r-0-1-comparison.md`) exercised `verify_worker_scope` for real against a live Worker's actual diff for the first time outside synthetic unit tests, and passed.
**Files changed:** `renmark/fast_path.py` (new), `renmark/providers/claude_agent.py`, `tests/test_fast_path.py` (new), `tests/test_r0_1_ux_regression_baseline.py` (new), `.bootstrap-renmark/milestones/R-0.1/**`, `.bootstrap-renmark/decisions/ADR-002-worker-scope-enforcement.md` (new), `.bootstrap-renmark/ledger/events.jsonl`, `VERSION`, `pyproject.toml`, `renmark/__init__.py`, `plugin/.claude-plugin/plugin.json`, `plugin/.codex-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `README.md`, `CHANGELOG.md`.
**Do not change:** `build_agent_dispatch` (non-fast-path) stays untouched — any future edit must keep `test_build_agent_dispatch_unaffected_by_fast_path_addition` green. No-nested-dispatch enforcement is honestly prompt-level only (F1 follow-up) — do not claim it is mechanically enforced without adding the missing host-transcript signal first. The R-0.0-vs-R-0.1 benchmark comparison is n=1, not a controlled A/B (F2) — do not cite it as proof of a cost improvement without a larger, controlled re-run.
