...
artifact_type: audit
schema_version: 1
created_at: 2026-07-15T20:00:02+00:00
...
| codereview | debug | gate | 2 | 377 | Use when the user wants a diff or PR reviewed — typed as /renmark:codereview or … |
...
| feature | build | pipeline | 2 | 299 | Use for the Feature pipeline (/renmark:feature) when adding to or changing an ex… |
...
| init | meta | aux | 2 | 243 | Use for the Project Setup pipeline (/renmark:init) to adopt renmark into a repo … |
...
| orchestrate | build | pipeline | 2 | 364 | Use to execute a renmark plan — `/renmark:orchestrate` or \"execute the plan\", … |
| plan | build | pipeline | 2 | 270 | Use when the user has a spec and wants it decomposed into an executable task lis… |
...
| start | build | pipeline | 2 | 267 | Use for the New Build pipeline (/renmark:start) when starting something new from… |
| usage | meta | aux | 2 | 86 | Use when the user wants observed local usage status — typed as /renmark:usage or… |
| verify | build | gate | 6 | 587 | Use after a build or `/renmark:orchestrate` to confirm it works — the post-build… |

## Summary

- 30 commands harvested from plugin/commands/*.md
- 30 have a backing SKILL.md, 0 missing
- domains: audit=3, build=13, debug=2, meta=12
- classes: aux=18, gate=2, pipeline=10
