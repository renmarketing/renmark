...
- (2026-08-07, bug, fixed) **summary.is_stale crashed on naive-vs-aware datetime compare** — a date-only `stale_after` string (e.g. `2026-09-05`, no timezone) parsed to a naive `datetime` that was then compared unconditionally against an aware `datetime.now(timezone.utc)`. Fixed by normalizing a naive parsed `stale_dt` to UTC before comparing. Surfaced again (after being logged-not-fixed on 2026-08-05) when `/renmark:finish`'s M6 re-verify actually hit the code path via `renmark.hygiene scan`. Lesson: a deferred "out of scope" bug can become load-bearing the moment routine ops exercise the same path.
...
- (2026-08-05, .renmark/reviews/2026-08-05-e03a8c3.verification.md) model `verify`: **verify-governed-orchestration-assurance-release-6** — 6/6 behaviors verified; failed: none; regressions: 0; full suite 2007 passed (up from 1983 baseline); F1 residual gap genuinely closed

- (2026-08-05, CHANGELOG.md 2026-08-05 rethink Release 6 wave 3) model `orchestrate`: **codex-task-ad-hoc-unreliable-not-just-extend** — REVISED finding (supersedes the release-4-task-5 note): renmark-execute --task <brief> --output <file> (codex ad-hoc single-task path) has now corrupted output 3 times this session with a bogus YAML/markdown artifact-report document instead of real code -- release-3 task5 (masked by accidental docstring wrap), release-4 task5 (genuine SyntaxError), release-6 task6 (genuine, new file this time, not an extend). This is NOT limited to extend-existing-file tasks as first suspected -- release-6 task5, a NEW file, worked fine in the same batch. Observed failure rate on this specific CLI path across ~7 attempts this session: roughly 50%, regardless of new-file vs extend-file. The whole-plan wave path (renmark-execute <plan>, no --task/--output) has never shown this failure mode. Revised recommendation: avoid the --task/--output ad-hoc path entirely for anything beyond trivial one-liners; prefer sonnet Agent-tool dispatch for codex-assigned tasks when using this path, or route codex tasks through the full-plan wave dispatch instead of ad-hoc single-task invocation. Always independently re-run the verifier and peek at the file head before trusting a PASS from this path.

- (2026-08-05, .renmark/reviews/2026-08-05-e54992d.verification.md) model `verify`: **verify-governed-orchestration-assurance-release-5** — 4/4 behaviors verified; failed: none; regressions: 0; full suite 1983 passed (up from 1980 baseline)

- (2026-08-05, .renmark/reviews/2026-08-05-3ae0f5c.verification.md) model `verify`: **verify-governed-orchestration-assurance-release-4** — 5/5 behaviors verified; failed: none; regressions: 0; full suite 1980 passed (up from 1974 baseline)
