...
- 🚨 **Oversized module: `renmark/lifecycle.py` (1100 code lines)** — `renmark/lifecycle.py` has 1100 code lines (threshold 1000). Large modules are hard to navigate, review, and test, and tend to accrete unrelated responsibilities. _Split `renmark/lifecycle.py` into focused modules along its natural seams (one cohesive responsibility per file). Advisory — never auto-refactored._
- 🚨 **High import fan-out: `renmark/lifecycle.py` (32 imports)** — `renmark/lifecycle.py` has 32 import statements (threshold 25). High fan-out signals a module coupled to many others — a change-amplifier and a refactoring hazard. _Reduce coupling in `renmark/lifecycle.py`: extract a narrower interface, group related imports behind a facade, or move logic closer to its dependencies. Advisory._
- 🚨 **Oversized module: `renmark/cli/_engine.py` (1278 code lines)** — `renmark/cli/_engine.py` has 1278 code lines (threshold 1000). Large modules are hard to navigate, review, and test, and tend to accrete unrelated responsibilities. _Split `renmark/cli/_engine.py` into focused modules along its natural seams (one cohesive responsibility per file). Advisory — never auto-refactored._
