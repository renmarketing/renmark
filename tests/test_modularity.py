...
def test_nesting_trips_cognitive_below_flat_branch_count(tmp_path: Path) -> None:
    ...
    assert raw_branches <= COGNITIVE_WARN, "test fixture invalid: flat equivalent would itself exceed cognitive warn"
    ...
    assert _gaps_for(repo_flat, title_contains="cognitive") == [], (
        "the same branch count laid out flat must NOT trip cognitive — this is the proof that nesting is weighted"
    )
...
def test_method_label_is_qualified_with_class_name(tmp_path: Path) -> None:
    ...
    assert not any(t.endswith("→ `run()`") for t in titles), f"bare 'run()' label found — must be qualified: {titles}"
