...
**Changelog check** — read the last 3 entries in `CHANGELOG.md`; flag any "Do not change" guards that overlap with the plan's target files. If an overlap is a genuine **contradiction** — the plan would undo or overwrite a guarded decision (not just touch the same file) — surface it and **PAUSE for reconciliation** before dispatching; never silently overwrite a recorded decision.
