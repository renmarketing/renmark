...
- The **eval tier** is the load-bearing behavioral proof — an LLM-as-judge over a real model trajectory (~$0.15, opt-in only, never auto-spends, out of CI). It runs through a subprocess runner: when `RENMARK_EVAL_RUNNER_CMD` (or the `.renmark` config key `eval_runner_cmd`) points at a `str->str` command, `renmark-execute --behavior --accept` records golden transcripts and `renmark-execute --behavior --judge` runs the live judge through that runner.
- When `RENMARK_EVAL_RUNNER_CMD`/`eval_runner_cmd` is UNSET the eval tier stays unavailable — CI-safe, deterministic-tier default, never auto-spends. On a deterministic FAIL the CLI prints an OFFER line and escalates solely when `--judge` is passed.
