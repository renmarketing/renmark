...
**Subagent profiles.** Prefer specialized dispatch roles (docs-editor, code-implementer, test-writer, reviewer, release-manager, researcher, audit-reader, finish-lane-specialist) over generic `general-purpose` agents. Every dispatch packet carries a `role` field; renmark logs and costs by role. Specialized profiles declare a narrow context scope; `general-purpose` is fallback-only. Claude definitions ship inside Renmark at `plugin/agents/` and dispatch as `renmark:<role>`; never require a global `~/.claude/agents/` copy. These are 8 plugin specialists plus Claude's built-in `general-purpose` ninth role, and only plan-relevant roles spawn on a run. See `CLAUDE.md` § `subagent-profiles-rule` + `plugin/skills/.shared/subagent-profiles.md`.
