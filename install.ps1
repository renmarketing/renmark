...
if ($Uninstall) {
    ...
    Write-Host "Run /reload-plugins in Claude Code to drop the slash commands."
    exit 0
}
...
# 4. Claude Code registry + settings.json (CRITICAL — must run after pip)
Write-Host ""
Write-Host "Registering with Claude Code (settings.json + installed_plugins.json):"
...
if ($pyExe -ne $null) {
    ...
    if ($canImport) {
        & $pyExe -m renmark.doctor --fix 2>&1 | Where-Object { $_ -match "^(Applying|  •|\[)" } | Select-Object -First 20
    } else {
...
Write-Host "  1. Restart Claude Code (or run /reload-plugins) so the new slash commands appear."
Write-Host "  2. Open a project folder in Claude Code and type:  /renmark:start"
Write-Host "  3. If /renmark:* commands don't appear, run:       /renmark:doctor"
Write-Host ""
