...
Next-step routing (``next_steps()``, ``next_recommended()``) lives in the
sibling module ``renmark/lifecycle/next_steps.py``.
"""
...
def milestone_signoff_ready(repo: Path | str) -> bool:
    ...
    # Function-local import: `reconciliation` imports this module at module
    # scope, so a top-level import here would be circular.
    from .reconciliation import milestone_signoff_readiness
...
def milestone_release_ready(repo: Path | str) -> bool:
    ...
    # Function-local import: see `milestone_signoff_ready` above.
    from .reconciliation import milestone_signoff_readiness
